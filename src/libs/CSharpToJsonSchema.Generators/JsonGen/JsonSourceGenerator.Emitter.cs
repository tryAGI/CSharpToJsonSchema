    ...
    public sealed partial class JsonSourceGenerator
    {
        private sealed partial class Emitter
        {
            ...
            public void Emit(ContextGenerationSpec contextGenerationSpec)
            {
                ...
                {
                    if(_typeIndex.TryAdd(spec.TypeRef, spec))
                        continue;
            ...
            private SourceText? GenerateTypeInfo(ContextGenerationSpec contextSpec, TypeGenerationSpec typeGenerationSpec)
            {
                switch (typeGenerationSpec.ClassType)
                {
                    ...
                    case ClassType.Object:
                        if(contextSpec.ContextType.Name == "CancellationToken")
            ...
            private static SourceText GetRootJsonContextImplementation(ContextGenerationSpec contextSpec, bool emitGetConverterForNullablePropertyMethod)
            {
                ...
                if (contextSpec.ContextType.TypeKind == TypeKind.Interface)
                {
                    var newName = contextTypeName.Substring(1)+"ExtensionsJsonSerializerContext";
                    var oldName = contextTypeName;
                    
                    contextTypeRef = contextTypeRef.Remove(contextTypeRef.LastIndexOf("."));

                    contextTypeRef = $"{contextTypeRef}.{newName}";
                    contextTypeName = newName;
                }
            ...
            private static void GetLogicForDefaultSerializerOptionsInit(SourceGenerationOptionsSpec? optionsSpec, SourceWriter writer)
            {
                ...
                static string FormatNamingPolicy(JsonKnownNamingPolicy knownNamingPolicy)
                {
                    string? policyName = knownNamingPolicy switch
                    {
                        ...
                        JsonKnownNamingPolicy.SnakeCaseLower => "SnakeCaseLower",
                        JsonKnownNamingPolicy.SnakeCaseUpper => "SnakeCaseUpper",
                        JsonKnownNamingPolicy.KebabCaseLower => "KebabCaseLower",
                        JsonKnownNamingPolicy.KebabCaseUpper => "KebabCaseUpper",
                        _ => null,
                    };
