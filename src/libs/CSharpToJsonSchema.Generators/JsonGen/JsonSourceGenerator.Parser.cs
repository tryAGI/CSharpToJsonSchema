...
using CSharpToJsonSchema.Generators.JsonGen.System.Text.Json;
using System.Collections.Immutable;
using System.Diagnostics;
using System.Diagnostics.CodeAnalysis;
using System.Text;
...
#pragma warning restore
            ...
            public ContextGenerationSpec? ParseContextGenerationSpec(ImmutableArray<((MethodDeclarationSyntax ContextClass, SemanticModel SemanticModel) Left, KnownTypeSymbols Right)> values, CancellationToken cancellationToken)
            {
                ...
                {
                    var (contextClassDeclaration, semanticModel) = l.Left;
                    nsList.Add(semanticModel.GetDeclaredSymbol(contextClassDeclaration, cancellationToken).ContainingNamespace.ToDisplayString());
                }

                var nameSpace = ToModels.GetCommonRootNamespace(nsList);
                INamedTypeSymbol contextTypeSymbol_ = null;
                var rootTypesToGenerate_ = new List<TypeToGenerate>();
                SourceGenerationOptionsSpec? options = null;
                INamedTypeSymbol? jsonSerializerContext = null;
                foreach (var l in values)
                {
                    var (contextClassDeclaration, semanticModel) = l.Left;
                    IMethodSymbol? contextTypeSymbol =
                        semanticModel.GetDeclaredSymbol(contextClassDeclaration, cancellationToken);
                    Debug.Assert(contextTypeSymbol != null);

                    contextTypeSymbol_ = contextTypeSymbol.ContainingType;//.ContainingType;
                    _contextClassLocation = contextClassDeclaration.GetLocation();
                    ...
                    (rootTypesToGenerate_??=new List<TypeToGenerate>()).AddRange(rootSerializableTypes ?? Enumerable.Empty<TypeToGenerate>());
                    ...
                    if (langVersion is null or < MinimumSupportedLanguageVersion)
                    {
                        ...
                        ReportDiagnostic(
                            global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                .JsonUnsupportedLanguageVersion, _contextClassLocation, langVersion?.ToDisplayString(),
                    ...
                    {
                        jsonSerializerContext = SymbolGenerator.GenerateToolJsonSerializerContext(
                            nameSpace, semanticModel.Compilation);
                        contextTypeSymbol_ = jsonSerializerContext;
                    }

                }
                
                if (!TryGetNestedTypeDeclarations(nameSpace, cancellationToken,
                         out List<string>? classDeclarationList))
                {
                    // Class or one of its containing types is not partial so we can't add to it.
                    // ReportDiagnostic(
                    //     global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                    //         .ContextClassesMustBePartial, _contextClassLocation, contextTypeSymbol.Name);
                    return null;
                }

                // Enqueue attribute data for spec generation
                foreach (TypeToGenerate rootSerializableType in rootTypesToGenerate_)
                {
                ...
                while (_typesToGenerate.Count > 0)
                {
                    ...
                    {
                        TypeGenerationSpec spec = ParseTypeGenerationSpec(typeToGenerate, contextTypeSymbol_, options);
                ...
                {
                    ContextType = new(jsonSerializerContext),
                    GeneratedTypes = _generatedTypes.Values.OrderBy(t => t.TypeRef.FullyQualifiedName)
            ...
            private static bool TryGetNestedTypeDeclarations(string nameSpace, CancellationToken cancellationToken,
                [NotNullWhen(true)] out List<string>? typeDeclarations)
            {
                typeDeclarations = new List<string>();
                typeDeclarations.Add("public partial class ToolsJsonSerializerContext");
                // typeDeclarations = null;
                //
                // for (TypeDeclarationSyntax? currentType = contextClassSyntax;
                //      currentType != null;
                //      currentType = currentType.Parent as TypeDeclarationSyntax)
                // {
                //     StringBuilder stringBuilder = new();
                //     bool isPartialType = false;
                //     bool isInterface = currentType is InterfaceDeclarationSyntax;
                ...
                //     }
                //
                //     if (!isPartialType && !isInterface)
                //     {
                //         typeDeclarations = null;
                //         return false;
                //     }
                //
                //     var kind = currentType.GetTypeKindKeyword();
                //     if (isInterface)
                //         kind = "partial class";
                //
                ...
                //
                //     string typeName = (typeSymbol.TypeKind == TypeKind.Interface)
                //         ? typeSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat) +
                //           "ExtensionsJsonSerializerContext"
                //         : typeSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
            ...
            public ContextGenerationSpec? ParseContextGenerationSpec(InterfaceDeclarationSyntax contextClassDeclaration,
                SemanticModel semanticModel, CancellationToken cancellationToken)
            {
                ...
                if (!TryGetNestedTypeDeclarations(contextClassDeclaration, semanticModel, cancellationToken,
                        out List<string>? classDeclarationList))
                {
                    // Class or one of its containing types is not partial so we can't add to it.
                    ReportDiagnostic(
                ...
                ContextGenerationSpec contextGenSpec = new()
                {
                    ...
                    GeneratedTypes = _generatedTypes.Values.OrderBy(t => t.TypeRef.FullyQualifiedName)
                        .ToImmutableEquatableArray(),
                    Namespace = contextTypeSymbol.ContainingNamespace is { IsGlobalNamespace: false } ns
                        ? ns.ToDisplayString()
                        : null,
                    ContextClassDeclarations = classDeclarationList.ToImmutableEquatableArray(),
                    GeneratedOptionsSpec = options,
            ...
            public ContextGenerationSpec? ParseContextGenerationSpec(ClassDeclarationSyntax contextClassDeclaration,
                SemanticModel semanticModel, CancellationToken cancellationToken)
            {
                if (!_compilationContainsCoreJsonTypes)
                {
            ...
            private static bool TryGetNestedTypeDeclarations(InterfaceDeclarationSyntax contextClassSyntax,
                SemanticModel semanticModel, CancellationToken cancellationToken,
                [NotNullWhen(true)] out List<string>? typeDeclarations)
            {
                typeDeclarations = null;
                     ...
                     currentType = currentType.Parent as TypeDeclarationSyntax)
                {
                    ...
                    string typeName = (typeSymbol.TypeKind == TypeKind.Interface)
                        ? typeSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat).Substring(1) +
                          "ExtensionsJsonSerializerContext"
                        : typeSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat);
                    stringBuilder.Append(typeName);
            ...
            //         Debug.Assert(typeSymbol != null);
            //
            //         string typeName = typeSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat)+"JsonSerializerContext";
            //         stringBuilder.Append(typeName);
            //
            //         (typeDeclarations ??= new()).Add(stringBuilder.ToString());
            //     }
            ...
            //     return true;
            // }
            private static bool TryGetNestedTypeDeclarations(ClassDeclarationSyntax contextClassSyntax,
                SemanticModel semanticModel, CancellationToken cancellationToken,
                [NotNullWhen(true)] out List<string>? typeDeclarations)
            {
                typeDeclarations = null;
                ...
                out SourceGenerationOptionsSpec? options)
            {
                ...
                if (contextClassSymbol.TypeKind == TypeKind.Interface)
                {
                    //
                }
            }
                ...
                out SourceGenerationOptionsSpec? options)
            {
                ...
                if (contextClassSymbol.TypeKind == TypeKind.Interface)
                {
                    foreach (var member in contextClassSymbol.GetMembers().OfType<IMethodSymbol>())
                    {
                        var type = SymbolGenerator.GenerateParameterBasedClassSymbol(
                            contextClassSymbol.ContainingNamespace.ToDisplayString(), member, compilation);

                        var typeToGenerate = new TypeToGenerate
                        {
                            Type = type,
                            Mode = null,
                            TypeInfoPropertyName = $"{member.Name}Args",
                            Location = type.GetLocation(),
                            AttributeLocation = null
                        };
                        ...
                        if (member.ReturnsVoid ||
                            member.ReturnType.MetadataName == "Task") //, || member.ReturnsByRefReadonly)
                            continue;
                        if (member.ReturnType.BaseType?.Name == "Task")
                        {
                            var types = (member.ReturnType as INamedTypeSymbol).TypeArguments[0];
                            var typeToGenerate3 = new TypeToGenerate
                            {
                                Type = types,
                                Mode = null,
                                TypeInfoPropertyName = $"{types.Name}",
                                Location = types.GetLocation(),
                                AttributeLocation = null
                            };
                            (rootSerializableTypes ??= new()).Add(typeToGenerate3);
                            continue;
                        }

                        var typeToGenerate2 = new TypeToGenerate
                        {
                            Type = member.ReturnType,
                            Mode = null,
                            TypeInfoPropertyName = $"{member.ReturnType.Name}",
                            Location = member.GetLocation(),
                            AttributeLocation = null
                        };
                        (rootSerializableTypes ??= new()).Add(typeToGenerate2);
                    }
                }

                options = new SourceGenerationOptionsSpec
                {
            ...
            private void ParseJsonSerializerContextAttributes(
                IMethodSymbol contextClassSymbol,
                Compilation compilation,
                string nameSpace,
                out List<TypeToGenerate>? rootSerializableTypes,
                out SourceGenerationOptionsSpec? options)
            {
                ...
                rootSerializableTypes = null;
                options = null;
                var member = contextClassSymbol;
                var type = SymbolGenerator.GenerateParameterBasedClassSymbol(
                    nameSpace, member, compilation);
                ...
                if (!member.ReturnsVoid && member.ReturnType.MetadataName != "Task") 
                {
                    if (member.ReturnType.BaseType?.Name == "Task")
                    {
                        var types = (member.ReturnType as INamedTypeSymbol).TypeArguments[0];
            ...
            //     // }
            //
            //     rootSerializableTypes ??= new();
            //
            //     if (contextClassSymbol.TypeKind == TypeKind.Interface)
            //     {
            //         foreach (var symbol in contextClassSymbol.GetMembers().OfType<IMethodSymbol>())
            //         {
            //             var symbol.Parameters[0].Type;
            //         }   
            //     }
            //     else
            //     {
            //         rootSerializableTypes.Add(new TypeToGenerate()
            //         {
            //             Type = contextClassSymbol,
            //             Mode = null,
            //             TypeInfoPropertyName = contextClassSymbol.Name,
            //             Location = contextClassSymbol.Locations.FirstOrDefault(),
            //             AttributeLocation = null
            //         });
            //     }
            //   
            //     // // Extract all properties of contextClassSymbol and add them to rootSerializableTypes
            //     // foreach (var member in contextClassSymbol.GetMembers())
            //     // {
            ...
            //     //             Mode = null,
            //     //             TypeInfoPropertyName = propertySymbol.Name,
            ...
            private SourceGenerationOptionsSpec ParseJsonSourceGenerationOptionsAttribute(INamedTypeSymbol contextType,
                AttributeData attributeData)
            {
                ...
                if (attributeData.ConstructorArguments.Length > 0)
                {
                    Debug.Assert(attributeData.ConstructorArguments.Length == 1 &
                                 attributeData.ConstructorArguments[0].Type?.Name is nameof(JsonSerializerDefaults));
                    ...
                    switch (namedArg.Key)
                    {
                        ...
                        case nameof(JsonSourceGenerationOptionsAttribute.Converters):
                            ...
                            foreach (TypedConstant element in namedArg.Value.Values)
                            {
                                ...
                                TypeRef? typeRef = GetConverterTypeFromAttribute(contextType, converterType,
                                    contextType, attributeData);
                                if (typeRef != null)
                                {
            ...
            private TypeGenerationSpec ParseTypeGenerationSpec(in TypeToGenerate typeToGenerate,
                INamedTypeSymbol contextType, SourceGenerationOptionsSpec? options)
            {
                Debug.Assert(IsSymbolAccessibleWithin(typeToGenerate.Type, within: contextType),
                ...
                if (type is { IsRefLikeType: true } or INamedTypeSymbol { IsUnboundGenericType: true }
                    or IErrorTypeSymbol)
                {
                    ...
                    else
                    {
                        if (type.CanUseDefaultConstructorForDeserialization(out IMethodSymbol? defaultCtor))
                        {
                            ...
                            constructorSetsRequiredMembers =
                                defaultCtor?.ContainsAttribute(_knownSymbols.SetsRequiredMembersAttributeType) == true;
                        }
                ...
                else
                {
                    ...
                    if (!TryGetDeserializationConstructor(type, useDefaultCtorInAnnotatedStructs,
                            out IMethodSymbol? constructor))
                    {
                        ReportDiagnostic(
                            global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                .MultipleJsonConstructorAttribute, typeToGenerate.Location, type.ToDisplayString());
                    }
                    // else if (constructor != null && !IsSymbolAccessibleWithin(constructor, within: contextType))
                    // {
                    //     ReportDiagnostic(
                    //         global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                    //             .JsonConstructorInaccessible, typeToGenerate.Location, type.ToDisplayString());
                    //     constructor = null;
                    // }
                    ...
                    ctorParamSpecs = ParseConstructorParameters(typeToGenerate, constructor, out constructionStrategy,
                        out constructorSetsRequiredMembers);
                    propertySpecs = ParsePropertyGenerationSpecs(contextType, typeToGenerate, options,
                        out hasExtensionDataProperty, out fastPathPropertyIndices);
                    propertyInitializerSpecs = ParsePropertyInitializers(ctorParamSpecs, propertySpecs,
                        constructorSetsRequiredMembers, ref constructionStrategy);
                }
                ...
                if (classType is ClassType.TypeUnsupportedBySourceGen)
                {
                    ReportDiagnostic(
                        global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                            .TypeNotSupported, typeToGenerate.AttributeLocation ?? typeToGenerate.Location,
                ...
                if (!_generatedContextAndTypeNames.Add((contextType.Name, typeInfoPropertyName)))
                {
                    ...
                    ReportDiagnostic(
                        global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                            .DuplicateTypeName, typeToGenerate.AttributeLocation ?? _contextClassLocation,
                ...
                return new TypeGenerationSpec
                {
                    ...
                    ImmutableCollectionFactoryMethod =
                        DetermineImmutableCollectionFactoryMethod(immutableCollectionFactoryTypeFullName),
                };
            }
                ...
                foreach (AttributeData attributeData in typeToGenerate.Type.GetAttributes())
                {
                                 ...
                                 _knownSymbols.JsonUnmappedMemberHandlingAttributeType))
                    {
                        unmappedMemberHandling =
                            (JsonUnmappedMemberHandling)attributeData.ConstructorArguments[0].Value!;
                        continue;
                    }
                    else if (SymbolEqualityComparer.Default.Equals(attributeType,
                                 _knownSymbols.JsonObjectCreationHandlingAttributeType))
                    {
                        objectCreationHandling =
                            (JsonObjectCreationHandling)attributeData.ConstructorArguments[0].Value!;
                        continue;
                    }
                    else if (!foundJsonConverterAttribute &&
                    ...
                    {
                        customConverterType =
                            GetConverterTypeFromJsonConverterAttribute(contextType, typeToGenerate.Type, attributeData);
                            ...
                            _knownSymbols.JsonDerivedTypeAttributeType))
                    {
                        ...
                        if (!isPolymorphic && typeToGenerate.Mode == JsonSourceGenerationMode.Serialization)
                        {
                            ReportDiagnostic(
                                global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                    .PolymorphismNotSupported, typeToGenerate.Location,
                ...
                out bool needsRuntimeType)
            {
                ...
                if (type.GetCompatibleGenericBaseType(_knownSymbols.IAsyncEnumerableOfTType) is INamedTypeSymbol
                ...
                else if ((actualTypeToConvert = type.GetCompatibleGenericBaseType(_knownSymbols.KeyedCollectionType)) !=
                         null)
                {
                ...
                else if ((actualTypeToConvert = type.GetCompatibleGenericBaseType(_knownSymbols.ListOfTType)) != null)
                {
                    ...
                    valueType = actualTypeToConvert.TypeArguments[0];
                             ...
                             type.GetCompatibleGenericBaseType(_knownSymbols.DictionaryOfTKeyTValueType)) != null)
                {
                ...
                else if (_knownSymbols.IsImmutableDictionaryType(type, out immutableCollectionFactoryTypeFullName))
                {
                    ...
                    valueType = genericArgs[1];
                             ...
                             type.GetCompatibleGenericBaseType(_knownSymbols.IDictionaryOfTKeyTValueType)) != null)
                {
                    ...
                    needsRuntimeType = SymbolEqualityComparer.Default.Equals(type, actualTypeToConvert);
                             ...
                             type.GetCompatibleGenericBaseType(_knownSymbols.IReadonlyDictionaryOfTKeyTValueType)) !=
                         null)
                {
                ...
                else if ((actualTypeToConvert = type.GetCompatibleGenericBaseType(_knownSymbols.ICollectionOfTType)) !=
                         null)
                {
                    collectionType = CollectionType.ICollectionOfT;
                    valueType = actualTypeToConvert.TypeArguments[0];
                ...
                else if ((actualTypeToConvert = type.GetCompatibleGenericBaseType(_knownSymbols.ConcurrentStackType)) !=
                         null)
                {
                    collectionType = CollectionType.ConcurrentStack;
                    valueType = actualTypeToConvert.TypeArguments[0];
                }
                else if ((actualTypeToConvert = type.GetCompatibleGenericBaseType(_knownSymbols.ConcurrentQueueType)) !=
                         null)
                {
                    collectionType = CollectionType.ConcurrentQueue;
                    valueType = actualTypeToConvert.TypeArguments[0];
                }
                else if ((actualTypeToConvert = type.GetCompatibleGenericBaseType(_knownSymbols.IEnumerableOfTType)) !=
                         null)
                {
                ...
                out List<int>? fastPathPropertyIndices)
            {
                ...
                {
                    if (currentType.Name == "CancellationToken")
                        continue;
                    ...
                    ref bool hasExtensionDataProperty)
                {
                    ...
                    AddPropertyWithConflictResolution(propertySpec, memberInfo, propertyIndex: properties.Count,
                        ref state);
            ...
            private ref struct PropertyHierarchyResolutionState(SourceGenerationOptionsSpec? options)
            {
                ...
                public Dictionary<string, (PropertyGenerationSpec, ISymbol, int index)> AddedProperties =
                    new(options?.PropertyNameCaseInsensitive == true
                ...
                ref PropertyHierarchyResolutionState state)
            {
                ...
                if (state.AddedProperties.TryAdd(propertySpec.EffectiveJsonPropertyName,
                ...
                else
                {
                    ...
                    else
                    {
                        bool ignoreCurrentProperty =
                            ...
                            (state.IgnoredMembers?.TryGetValue(memberName, out ISymbol? ignored) == true &&
            ...
            private bool IsValidDataExtensionPropertyType(ITypeSymbol type)
            {
                ...
                INamedTypeSymbol? actualDictionaryType =
                    type.GetCompatibleGenericBaseType(_knownSymbols.IDictionaryOfTKeyTValueType);
                ...
                SourceGenerationOptionsSpec? options)
            {
                ...
                if (isExtensionData)
                {
                    if (typeHasExtensionDataProperty)
                    {
                        ReportDiagnostic(
                            global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                .MultipleJsonExtensionDataAttribute, typeLocation, declaringType.Name);
                    }
                    ...
                    if (!IsValidDataExtensionPropertyType(memberType))
                    {
                        ReportDiagnostic(
                            global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                .DataExtensionPropertyInvalid, memberInfo.GetLocation(), declaringType.Name,
                            memberInfo.Name);
                    }
                ...
                if (memberType.IsRefLikeType)
                {
                    ...
                    if (ignoreCondition is not JsonIgnoreCondition.Always)
                    {
                        ReportDiagnostic(
                            global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                .TypeContainsRefLikeMember, memberInfo.GetLocation(), declaringType.Name,
                            memberInfo.Name);
                    }
                ...
                string effectiveJsonPropertyName =
                    DetermineEffectiveJsonPropertyName(memberInfo.Name, jsonPropertyName, options);
                ...
                return new PropertyGenerationSpec
                {
                    NameSpecifiedInSourceCode =
                        memberInfo.MemberNameNeedsAtSign() ? "@" + memberInfo.Name : memberInfo.Name,
                ...
                out bool hasJsonRequiredAttribute)
            {
                ...
                foreach (AttributeData attributeData in memberInfo.GetAttributes())
                {
                    ...
                    {
                        converterType =
                            GetConverterTypeFromJsonConverterAttribute(contextType, memberInfo, attributeData);
                    }
                ...
                out bool constructorSetsRequiredMembers)
            {
                ...
                constructorSetsRequiredMembers =
                    constructor?.ContainsAttribute(_knownSymbols.SetsRequiredMembersAttributeType) == true;
                ...
                else
                {
                    ...
                    for (int i = 0; i < paramCount; i++)
                    {
                        ...
                        if (parameterInfo.Type.IsRefLikeType)
                        {
                            ReportDiagnostic(
                                global::CSharpToJsonSchema.Generators.JsonGen.JsonSourceGenerator.DiagnosticDescriptors
                                    .TypeContainsRefLikeMember, parameterInfo.GetLocation(), type.Name,
                                parameterInfo.Name);
                            constructionStrategy = ObjectConstructionStrategy.NotApplicable;
                            continue;
                        }
                ...
                ref ObjectConstructionStrategy constructionStrategy)
            {
                ...
                foreach (PropertyGenerationSpec property in properties)
                {
                    ...
                    if ((property.IsRequired && !constructorSetsRequiredMembers) || property.IsInitOnlySetter)
                    {
                        ...
                        static ParameterGenerationSpec? GetMatchingConstructorParameter(PropertyGenerationSpec propSpec,
            ...
            private TypeRef? GetConverterTypeFromJsonConverterAttribute(INamedTypeSymbol contextType,
                ISymbol declaringSymbol, AttributeData attributeData)
            {
            ...
            private TypeRef? GetConverterTypeFromAttribute(INamedTypeSymbol contextType, ITypeSymbol? converterType,
                ISymbol declaringSymbol, AttributeData attributeData)
            {
                        ...
                        c.Parameters.Length == 0 && IsSymbolAccessibleWithin(c, within: contextType)))
                {
                    ReportDiagnostic(
                        ...
                        converterType?.ToDisplayString() ?? "null", declaringSymbol.ToDisplayString());
                    return null;
                }
            ...
            private static string DetermineEffectiveJsonPropertyName(string propertyName, string? jsonPropertyName,
                SourceGenerationOptionsSpec? options)
            {
                ...
                out IMethodSymbol? deserializationCtor)
            {
                ...
                IMethodSymbol[] publicCtors = namedType.GetExplicitlyDeclaredInstanceConstructors()
                    .Where(ctor => ctor.DeclaredAccessibility is Accessibility.Public).ToArray();
                ...
                foreach (IMethodSymbol constructor in namedType.GetExplicitlyDeclaredInstanceConstructors()
            ...
            private bool IsSymbolAccessibleWithin(ISymbol symbol, INamedTypeSymbol within)
                =>
                    true; // symbol.Name.EndsWith("Args") || _knownSymbols.Compilation.IsSymbolAccessibleWithin(symbol, within);
